...
## To access : http://localhost:8000/orienters/general/fix_connector_report?g=1
def fix_connector_report(request):
    status = 'ko'

    g = request.GET.get('g',0)
    if g is not 0:
        reports = M_O_D_E_L_S.ConnectorReport.objects.all()

        for report in reports:
            # print(report)
            if report.stage == 'identifier' or report.stage == 'illuminator':
                ## do nothing
                print('row with id {0:03d} is ok'.format(report.id))
            else:
                ## swap the incorrectly set columns :P
                _s = report.stage
                report.stage = report.target
                report.target = _s
                report.save()
                print('row with id {0:03d} updated'.format(report.id))
